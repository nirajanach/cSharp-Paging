...
//Extension chunk method for IQueryable<T>
public static class IQueryableExtensions
{
    public static IEnumerable<IQueryable<T>> Chunk<T>(this IQueryable<T> query, int chunkSize)
    {
        for (int skip = 0; skip < query.Count(); skip += chunkSize)
        {
            yield return query.Skip(skip).Take(chunkSize);
        }
    }
}
...
partial class Program
{
    static void FilterAndSort()
    {
        ...
        using (Northwind db = new())
        {
            ...
            IOrderedQueryable<Product> sortedAndFilteredProducst = 
                filteredProducts.OrderByDescending(product => product.UnitPrice);

            var projectedProducts = sortedAndFilteredProducst
            ...
            WriteLine();
            WriteLine(sortedAndFilteredProducst.ToQueryString());
    ...
    {
        SectionTitle("Group Join Categories and Product");
    ...
    //        //use the chunk method to split data into pages
    //        var pages = pagingQuery.Chunk(pageSize);




    //    //.Skip(currentPage * pageSize).Take(pageSize);
    //    SectionTitle(pages.ToQueryString());
        ...
        int pageSize, int currentPage, int totalPages)
    {
        ...
        // Use the Chunk method to split data into pages
        var pages = pagingQuery.Chunk(pageSize).ToList();

        SectionTitle(pages[currentPage].ToQueryString());

        OutputTableOfProducts(pages[currentPage].ToArray(), currentPage, totalPages);
    }
